...
{
    [Area("Provider")]
    public class PropertiesController : ProviderCoreController
    {
        ...
        public IActionResult Edit(int id)
        {
            ...
            Property? property = _context.Properties
                            .Where(property => property.OwnerLevel == 2 && property.OwnerUserId == userId && property.Id == id && property.IsDeleted == 0)
                            .FirstOrDefault();
            if (property == null)
            {
                return NotFound();
        ...
        public async Task<IActionResult> Update(PropertyUpdateActionModel payloadData)
        {
            if (ModelState.IsValid)
            {
                ...
                .Where(property => property.OwnerLevel == 2 && property.OwnerUserId == userId && property.Id == payloadData.Id && property.IsDeleted == 0)
                .FirstOrDefault();
        ...
        public IActionResult Delete(int id)
        {
            int userId = GetUserId();
            Property? check_property_exists = _context.Properties
                                .Where(property => property.OwnerLevel == 2 && property.OwnerUserId == userId && property.IsDeleted == 0 && property.Id == id)
                                .FirstOrDefault();
            if (check_property_exists == null)
            {
                return Json(new { success = false, message = "Something Went Wrong" });
            ...
            {
                check_property_exists.IsDeleted = 1;
                _context.Properties.Update(check_property_exists);
                _context.SaveChanges();
